/* ─────────────────────────────────────────────────────────
   modals.js — Share, Search, and README modals
   Includes fuzzy search, Did-You-Mean, filter syntax,
   keyboard navigation, focus trapping, and XSS sanitisation.
   ───────────────────────────────────────────────────────── */

/* ── XSS defence: strip event-handler attributes & dangerous tags ── */
...
(function () {
    ...
    '<div id="search-modal" class="hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-sm p-4 sm:p-10 justify-center items-start" role="dialog" aria-modal="true" aria-label="Search documentation">',
      ...
      '<div class="theme-bg border theme-border w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col overflow-hidden mt-10 md:h-[550px] max-h-[85vh]">',
        ...
        '<div class="flex flex-1 overflow-hidden min-h-0">',
          '<div id="modal-search-results" class="w-full md:w-1/2 p-4 overflow-y-auto flex flex-col gap-2 border-r theme-border" role="listbox" aria-label="Search results"></div>',
          '<div id="modal-search-preview" class="hidden md:flex md:w-1/2 p-6 overflow-y-auto flex-col theme-bg theme-text min-w-0"></div>',
        '</div>',
  ...
  /* Only inject search modal if a ".open-search-btn" exists on the page */
  ...
  var _lastFocusedEl = null; /* restore focus on close */

  /* Open share modal — populate URL + set share links */
  document.addEventListener('click', function (e) {
    ...
    /* fill each share link's href from data-href template */
  ...
  /* Escape / focus-trap for share modal */
  ...
  /* ────────────────────────────────────────────
     SEARCH MODAL LOGIC (only if search btn exists)
     ──────────────────────────────────────────── */
  ...
  function _getFocusable(el) {
    ...
    return Array.from(el.querySelectorAll(sel)).filter(function (f) { return f.offsetParent !== null; });
  }
  ...
  /* ── Search history (last 5 queries, persisted) ── */
  function _getSearchHistory() {
    try { return JSON.parse(localStorage.getItem('__GB_SEARCH_HISTORY')) || []; } catch (_) { return []; }
  }
  ...
  function _clearSearchHistory() {
    try { localStorage.removeItem('__GB_SEARCH_HISTORY'); } catch (_) {}
  }

  /* ── Levenshtein distance (used by fuzzy scorer) ── */
  ...
  /* ── Fuzzy score (0–1) for a single field against a query ── */
  ...
  /* ── Dice coefficient (bigram overlap) for Did-You-Mean ── */
  ...
  function _diceSimilarity(a, b) {
    ...
    var bb = _bigramSet(b);
    var inter = 0;
    for (var k in ba) { if (ba.hasOwnProperty(k) && bb.hasOwnProperty(k)) inter++; }
    var total = Object.keys(ba).length + Object.keys(bb).length;
    return total === 0 ? 0 : (2 * inter) / total;
  }

  /* Suggest an alternative search term if no results found */
  function _didYouMean(query, candidates) {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      if (score > bestScore) { bestScore = score; best = candidates[i].title; }
      if (candidates[i].tags) {
        for (var t = 0; t < candidates[i].tags.length; t++) {
          ...
          if (tagScore > bestScore) { bestScore = tagScore; best = candidates[i].tags[t]; }
        }
      }
    }
  ...
  /* ── Parse query: extract filter tokens (cat:, tag:, section:) from plain terms ── */
  ...
  /* Check whether an item matches all active filters */
  ...
  /* ── Score an item against the query (weighted sum of field matches) ── */
  function _scoreItem(item, query) {
    var q = (query || '').toLowerCase().trim();
    if (!q) return 0;
    var qWords = q.split(/\s+/).filter(Boolean);

    /* concatenate all searchable fields (including cheat-sheet topics) */
    var textToSearch = (
      (item.title || '') + ' ' +
      (item.category || '') + ' ' +
      (item.description || '') + ' ' +
    ...
    /* quick rejection: if no word overlaps, skip */
    var hasOverlap = false;
    for (var k = 0; k < qWords.length; k++) {
      if (textToSearch.indexOf(qWords[k]) !== -1) { hasOverlap = true; break; }
    }
    if (!hasOverlap && q.length >= 3) return 0;

    /* weighted scores per field */
    ...
    /* multi-field bonus */
    var matched = 0;
    if (titleScore > 0) matched++;
    if (catScore > 0) matched++;
    if (descScore > 0) matched++;
    if (bestSectionScore > 0) matched++;
    if (bestTagScore > 0) matched++;
    var bonus = matched >= 3 ? 15 : matched >= 2 ? 5 : 0;
  ...
  function _highlight(text, query) {
    ...
    return text.replace(re, function (match, p1) {
      return p1 ? '<mark class="bg-brand-100 text-inherit rounded-sm px-0.5">' + p1 + '</mark>' : match;
    });
  }
  ...
  function updateSearchPreview(item) {
    ...
    if (!item) {
      preview.innerHTML = '<div class="flex flex-col items-center justify-center h-full text-center p-6 theme-text-muted">' +
        '<span class="material-symbols-outlined text-4xl mb-2">find_in_page</span>' +
        '<p class="text-xs font-medium">Select a guide to preview content details</p></div>';
      return;
    }

    var sectionsHtml = '';
    if (item.sections && item.sections.length > 0) {
      sectionsHtml = '<div class="mt-4">' +
        ...
        item.sections.map(function (sec) {
          return '<li class="flex items-center gap-2 text-xs theme-text-muted truncate">' +
    ...
    if (item.tags && item.tags.length > 0) {
      tagsHtml = '<div class="flex flex-wrap gap-1.5 mt-4">' +
    ...
    /* close modal when preview "Read Study Guide" is clicked */
    var btn = preview.querySelector('.select-guide-btn');
    if (btn) { btn.addEventListener('click', function () { closeSearchModal(); }); }
  }
  ...
  function renderResults(rawQuery) {
    ...
    try {
      ...
      /* empty query → show history / category browse */
      if (!q) {
        _currentResults = [];
      ...
      /* apply filters, then score + rank */
      var filtered = searchIndex.filter(function (item) { return _matchesFilters(item, filters); });
      ...
      if (searchTerms.length >= 2 && topItems.length > 0) _addSearchHistory(searchTerms);

      _currentResults = topItems.map(function (s) { return s.item; });

      var announcer = document.getElementById('search-live-announcer');
      if (announcer) announcer.textContent = topItems.length === 0 ? 'No results found.' : topItems.length + ' results found.';
      ...
      html += topItems.map(function (scored, idx) {
        ...
        return '<a href="' + item.url + '" role="option" class="search-result flex items-center justify-between p-3 theme-bg hover:bg-brand-50 border theme-border hover:border-brand-100 rounded-xl transition-all group" data-index="' + idx + '">' +
      ...
      if (firstLink) { _applySelection(firstLink); }
  ...
  /* ── Empty-state: recent searches + category chips ── */
  function renderEmptyState(results) {
    ...
    if (history.length > 0) {
      html += '<div class="mb-4">' +
        '<div class="flex items-center justify-between mb-2">' +
          '<span class="text-xs font-semibold uppercase tracking-wider text-slate-400">Recent</span>' +
          '<button id="clear-search-history-btn" class="text-[10px] text-slate-400 hover:text-red-500 transition-colors font-mono uppercase tracking-wider">Clear</button>' +
        '</div>' +
    ...
    html += '<div class="mb-3"><span class="text-xs font-semibold uppercase tracking-wider text-slate-400">Browse by category</span></div>' +
      '<div class="flex flex-wrap gap-2">';
    var cats = Object.keys(categories).sort();
    for (var c = 0; c < cats.length; c++) {
      html += '<button data-category="' + _escapeRe(cats[c]) + '" class="category-chip text-xs theme-text theme-bg hover:bg-brand-50 px-2.5 py-1 rounded-lg transition-colors">' + cats[c] + ' <span class="text-slate-400">(' + categories[cats[c]] + ')</span></button>';
    }
    html += '</div>';
  ...
  /* ── No-results state: suggest Did-You-Mean ── */
  function renderNoResults(results, queryTerms, filters, rawQuery) {
    var html = '<div class="p-8 text-center font-mono">' +
      '<span class="material-symbols-outlined text-3xl theme-text-muted mb-3" aria-hidden="true">search_off</span>' +
      '<p class="theme-text-muted text-sm mb-1">No matches for <span class="theme-text font-bold">&ldquo;' + _escapeRe(queryTerms) + '&rdquo;</span></p>' +
      '<p class="text-xs theme-text-muted mb-4">Try a broader term or check your spelling.</p>';
    ...
    if (rawQuery.indexOf(':') !== -1) {
      html += '<div class="px-4 pb-2 text-center"><p class="text-[10px] text-slate-400 font-mono">Filter syntax: <span class="text-brand-500">cat:git</span> <span class="text-slate-500">or</span> <span class="text-brand-500">tag:staff+</span></p></div>';
    }
  ...
  function _navigateSearch(dir) {
    ...
    if (_selectedIndex >= 0 && links[_selectedIndex]) _clearSelection(links[_selectedIndex]);
    ...
    if (_currentResults[_selectedIndex]) updateSearchPreview(_currentResults[_selectedIndex]);
  }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
      e.preventDefault();
      var isOpen = searchModal && !searchModal.classList.contains('hidden');
  ...
  document.addEventListener('input', function (e) {
    ...
    if (!value || value.trim().length === 0) { renderResults(value); return; }
    _searchDebounce = setTimeout(function () { renderResults(value); }, 150);
  });
  ...
  document.addEventListener('keydown', function (e) {
    ...
    switch (e.key) {
      case 'ArrowDown': e.preventDefault(); _navigateSearch(1); break;
      case 'ArrowUp': e.preventDefault(); _navigateSearch(-1); break;
      case 'Enter': {
        var links = document.querySelectorAll('#modal-search-results .search-result');
  ...
  /* Hover over a result → select it + update preview */
  ...
  function loadAndRenderReadme() {
    fetch('README.md')
      ...
      .then(function (text) {
        ...
        var content = document.getElementById('readme-content');
        if (content) { content.innerHTML = html; readmeLoaded = true; }
      })
      .catch(function (err) {
        console.error(err);
        var content = document.getElementById('readme-content');
